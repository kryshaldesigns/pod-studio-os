      ...
      {items.map((item, i) => (
        <div key={i} onClick={() => toggle(i)} style={{ display: "flex", alignItems: "flex-start", gap: 10, padding: "8px 0", borderBottom: "1px solid #1e1e1e", cursor: "pointer" }}>
          <div style={{ width: 18, height: 18, borderRadius: 4, border: `2px solid ${checked.includes(i) ? accent.main : "#333"}`, background: checked.includes(i) ? accent.main : "transparent", flexShrink: 0, marginTop: 1, display: "flex", alignItems: "center", justifyContent: "center" }}>
            {checked.includes(i) && <span style={{ color: "#080c14", fontSize: 11, fontWeight: 900 }}>✓</span>}
          </div>
          ...
          {notifications.length === 0 ? <div style={{ color: "#555", fontSize: 12, textAlign: "center", padding: 12 }}>No notifications</div>
            : notifications.map(n => (
              <div key={n.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", padding: "8px 0", borderBottom: "1px solid #222" }}>
                ...
                <button onClick={() => onClear(n.id)} style={{ background: "none", border: "none", color: "#555", cursor: "pointer", fontSize: 14, padding: "0 4px" }}>×</button>
              </div>
            ))}
        </div>
      )}
...
const FounderDashboard = ({ state, update, addNotification, clearNotification }) => {
  ...
  const run = async (prompt, notify) => {
    ...
    setLoading(false);
    if (notify) addNotification(notify, notify === "designer" ? "New brief pushed from Dad 📋" : "New listing copy ready 📝");
    return res;
  };
    ...
    <div>
        ...
        <div>
          ...
          {output && (
            <div style={{ marginTop: 8 }}>
              <Btn accent={accent} small secondary onClick={() => { update("nicheBriefs", [{ id: Date.now(), content: output, week: new Date().toDateString(), status: "new" }, ...state.nicheBriefs]); addNotification("designer", "New design brief is ready — check your dashboard 📋"); }}>
        ...
        <div>
          ...
          {output && (
            <div style={{ marginTop: 8, display: "flex", gap: 8 }}>
              <Btn accent={accent} small secondary onClick={() => { update("listingCopies", [{ id: Date.now(), niche: listingNiche, content: output, status: "pending_upload" }, ...state.listingCopies]); addNotification("designer", "Merch listing copy ready for upload 📝"); }}>
                Send to Son
              </Btn>
      ...
      {activeTab === "rejection" && (
        <div>
          ...
          <OutputBox content={output} accent={accent} />
          {output && (
            <Btn accent={accent} small secondary style={{ marginTop: 8 }} onClick={() => { addNotification("designer", "Rejection diagnosed — corrected copy ready 🔧"); }}>
              Send Fix to Son
            </Btn>
          )}
        </div>
      ...
      {activeTab === "research" && (
        <div>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            ...
            <Btn accent={accent} secondary small disabled={!trendDesc} onClick={() => { addNotification("founder", `🚨 Urgent trend flagged by wife: "${trendDesc.slice(0, 60)}..."`); setTrendDesc(""); }}>
          ...
          <OutputBox content={output} accent={accent} />
          {output && (
            <Btn accent={accent} small secondary style={{ marginTop: 8 }} onClick={() => { update("trendReports", [{ id: Date.now(), content: output, week: new Date().toDateString() }, ...state.trendReports]); addNotification("founder", "📊 Weekly trend report submitted by wife — ready to review"); setOutput(""); setFindings(["", "", "", "", ""]); }}>
              Submit to Husband ✓
            </Btn>
          )}
      ...
      {activeTab === "redbubble" && (
        <div>
          ...
          {state.designHandoffs.filter(h => h.rb_status === "pending_rb_listings").length > 0 && (
            <Alert accent={accent}>
              📥 {state.designHandoffs.filter(h => h.rb_status === "pending_rb_listings").length} design handoff(s) waiting for Redbubble listings
            </Alert>
          )}
          ...
          <OutputBox content={output} accent={accent} />
          {output && (
            <Btn accent={accent} small secondary style={{ marginTop: 8 }} onClick={() => { addNotification("designer", "Redbubble listing copy ready for upload 📋"); setOutput(""); }}>
              Mark Done — Notify Son ✓
            </Btn>
          )}
    ...
    <div>
      ...
      {incomingPattern && (
        <div style={{ background: accent.dim, border: `1px solid ${accent.border}`, borderRadius: 10, padding: "8px 12px", marginBottom: 12, fontSize: 12, color: accent.text }}>
          ✓ Pattern received from Pattern Press Generator
        </div>
      )}
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          ...
          {activePattern && <Badge label={uploadedPattern ? "Custom pattern loaded ✓" : "Pattern Press pattern loaded ✓"} color={accent.main} />}
        </div>
        ...
        <div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
            <Card accent={accent} style={{ padding: 14 }}>
              ...
              {[["Format", "PNG"], ["Size", "4500 × 5400px"], ["Colour", "RGB"], ["DPI", "300"], ["Max file", "25MB"]].map(([k, v]) => (
      ...
      {activeTab === "handoff" && (
        <div>
          ...
          <Btn accent={accent} disabled={!handoffNiche || !handoffDesigns[0].name} onClick={() => {
            ...
            addNotification("operations", `📥 ${handoffDesigns.filter(d => d.name).length} designs handed off by son — Redbubble listings needed (${handoffNiche})`);
            addNotification("founder", `📥 ${handoffDesigns.filter(d => d.name).length} designs completed by son — Merch listing copy needed (${handoffNiche})`);
            setHandoffNiche(""); setHandoffDesigns([{ name: "", description: "", designText: "", style: "Text-based" }]);
            alert("✓ Handed off! Mum and Dad have been notified.");
          }}>
            Submit Handoff ✓
          </Btn>
        </div>
